...
# Install globally install specific pnpm version
...
RUN adduser --system --uid 1001 nivla-ai
...
COPY --from=builder --chown=nivla-ai:nodejs /builder/node_modules /runner/node_modules
COPY --from=builder --chown=nivla-ai:nodejs /builder/dist /runner/dist
COPY --from=builder --chown=nivla-ai:nodejs /builder/package.json /runner/package.json
